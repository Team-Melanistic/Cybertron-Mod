...
public class TeleporterCybertron extends Teleporter
{
			...
			for (i3 = -1; i3 <= 1; ++i3)
			{
				for (j3 = 1; j3 < 3; ++j3)
				{
					for (k3 = -1; k3 < 3; ++k3)
					{
						...
						j4 = k2 + (j3 - 1) * l2 - i3 * l5;
						flag = k3 < 0;
						worldServerInstance.setBlock(l3, i4, j4, flag ? CyberBlocks.concrete : Blocks.air);
					}
				}
			}
		}
		...
		for (i3 = 0; i3 < 4; ++i3)
		{
			for (j3 = 0; j3 < 4; ++j3)
			{
				for (k3 = -1; k3 < 4; ++k3)
				{
					...
					worldServerInstance.setBlock(l3, i4, j4, flag ? CyberBlocks.concrete : CyberBlocks.cyberPortal, 0, 2);
				}
			}
	...
	public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
	{
		...
		{
			TeleporterCybertron.PortalPosition portalposition = (TeleporterCybertron.PortalPosition) destinationCoordinateCache.getValueByKey(j1);
			d3 = 0.0D;
		...
		else
		{
			for (l3 = l - short1; l3 <= l + short1; ++l3)
			{
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
				{
					...
					{
						if (worldServerInstance.getBlock(l3, i2, l1) == CyberBlocks.cyberPortal)
						{
							while (worldServerInstance.getBlock(l3, i2 - 1, l1) == CyberBlocks.cyberPortal)
							{
								--i2;
							}
		...
		if (d3 >= 0.0D)
		{
			if (flag)
			{
				destinationCoordinateCache.add(j1, new TeleporterCybertron.PortalPosition(i, j, k, worldServerInstance.getTotalWorldTime()));
			...
			if (worldServerInstance.getBlock(i - 1, j, k) == CyberBlocks.cyberPortal)
			{
				i4 = 2;
			}
			...
			if (worldServerInstance.getBlock(i, j, k - 1) == CyberBlocks.cyberPortal)
			{
				i4 = 3;
			}
			...
			for (int l = -2; l <= 2; ++l)
			{
				for (int i1 = -2; i1 <= 2; ++i1)
				{
					for (int j1 = -1; j1 < 3; ++j1)
					{
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						worldServerInstance.setBlock(k1, l1, i2, flag ? CyberBlocks.concrete : Blocks.air);
					}
				}
			}
	...
	public void removeStalePortalLocations(long par1)
	{
		if (par1 % 100L == 0L)
		{
			...
			while (iterator.hasNext())
			{
				...
				TeleporterCybertron.PortalPosition portalposition = (TeleporterCybertron.PortalPosition) destinationCoordinateCache.getValueByKey(olong.longValue());
